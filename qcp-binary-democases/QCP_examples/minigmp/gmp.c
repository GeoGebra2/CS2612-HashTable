...
/*@ neg
  ...
  Require
    ...
    Zabs(old) <= cap &&
    old < 0 && m < 0 && mpd_store_Z_compact(UINT_MOD,ptr, -m, -old) * UIntArray::undef_ceil(ptr, -old, cap) &&
      z -> _mp_size == old &&
      z -> _mp_alloc == cap &&
      z -> _mp_d == ptr
  Ensure
    m < 0 && mpd_store_Z_compact(UINT_MOD,__return, -m, -old) * UIntArray::undef_ceil(__return, -old, Z::max(Z::max(n,1),cap)) &&
    z -> _mp_size == old &&
