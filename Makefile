...
PL_FLAG = -R $(PL_DIR) PL -R $(HashTable_DIR) PL.HashTable
...
DEP_FLAG = -R $(PL_DIR) PL -R $(HashTable_DIR) PL.HashTable -R $(QCP_DIR)/SeparationLogic SimpleC.SL -R $(QCP_DIR)/unifysl Logic -R $(QCP_DIR)/sets SetsClass -R $(QCP_DIR)/compcert_lib compcert.lib -R $(QCP_DIR)/auxlibs AUXLib -R $(QCP_DIR)/examples SimpleC.EE -R $(QCP_DIR)/StrategyLib SimpleC.StrategyLib -R $(QCP_DIR)/Common SimpleC.Common -R $(QCP_DIR)/fixedpoints FP -R $(QCP_DIR)/MonadLib MonadLib
